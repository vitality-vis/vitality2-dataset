#!/usr/bin/env python3
"""Batch-fetch PDF URL candidates from OpenAlex Work locations by DOI."""
...
from fetch_arxiv_fullpaper_md import normalize_doi, normalize_text  # noqa: E402


OPENALEX_WORKS_URL = "https://api.openalex.org/works"
SELECT_FIELDS = "id,doi,title,primary_location,best_oa_location,locations,open_access"
...
def source_id(location: dict[str, Any]) -> str:
    source = location.get("source")
    if not isinstance(source, dict):
        return ""
    return normalize_text(source.get("id"))


def pdf_candidates_from_work(work: dict[str, Any]) -> list[dict[str, Any]]:
    candidates: list[dict[str, Any]] = []
    seen: set[str] = set()

    def add(location: Any, location_kind: str) -> None:
        ...
        candidates.append(
            {
                "pdf_url": pdf_url,
                "landing_page_url": normalize_text(location.get("landing_page_url")),
                "location_kind": location_kind,
                "version": normalize_text(location.get("version")),
                "license": normalize_text(location.get("license")),
                "is_oa": bool(location.get("is_oa")),
                "source_name": source_name(location),
                "source_id": source_id(location),
            }
        )

    add(work.get("best_oa_location"), "best_oa_location")
    add(work.get("primary_location"), "primary_location")
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if 500 <= response.status_code < 600 and attempt < max_retries:
                ...
                if verbose:
                    print(
                        f"OpenAlex batch HTTP {response.status_code}; retry {attempt + 1}/{max_retries} after {delay:g}s",
                        file=sys.stderr,
                        flush=True,
                    )
                time.sleep(delay)
            ...
            if attempt < max_retries:
                ...
                if verbose:
                    print(
                        f"OpenAlex batch request failed: {exc}; retry {attempt + 1}/{max_retries} after {delay:g}s",
                        file=sys.stderr,
                        flush=True,
                    )
                time.sleep(delay)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prefetch OpenAlex PDF URLs in DOI batches.")
    ...
    parser.add_argument("--user-agent", default="Vitality2 OpenAlex PDF URL fetcher (mailto:unknown@example.com)")
...
def run(args: argparse.Namespace) -> int:
    ...
    doi_to_record: dict[str, dict[str, Any]] = {}
    dois: list[str] = []
    for record in records:
        doi = normalize_doi(record.get("doi"))
        key = doi.casefold()
        if not doi or key in doi_to_record:
            continue
        doi_to_record[key] = record
        dois.append(doi)
    ...
    with args.output.open("a", encoding="utf-8") as out:
        for batch in chunks(pending, args.batch_size):
            ...
            for doi in batch:
                processed += 1
                key = doi.casefold()
                source_record = doi_to_record.get(key) or {}
                work = returned.get(key)
                if work is not None:
                    candidates = pdf_candidates_from_work(work)
                    item = {
                        "paper_uid": source_record.get("paper_uid") or f"doi:{doi}",
                        "doi": doi,
                        "title": source_record.get("title") or work.get("title") or "",
                        "service": "openalex",
                        "status_code": 200,
                        "openalex_id": work.get("id") or "",
                        "pdf_status": "found" if candidates else "not_found",
                        "best_pdf_url": candidates[0]["pdf_url"] if candidates else "",
                        "pdf_urls": candidates,
                        "open_access": work.get("open_access") or {},
                    }
                    counts["found" if candidates else "not_found"] += 1
                elif error:
                    item = {
                        "paper_uid": source_record.get("paper_uid") or f"doi:{doi}",
                        "doi": doi,
                        "title": source_record.get("title") or "",
                        "service": "openalex",
                        "status_code": 0,
                        "pdf_status": "failed",
                        "best_pdf_url": "",
